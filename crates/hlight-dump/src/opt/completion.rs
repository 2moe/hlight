...
/// It takes a generic type parameter G which specifies the type of shell to
/// generate the script for, and returns a byte vector containing the generated
/// script.
...
/// Generates a shell completion script based on the specified shell type in
/// args, and prints it out.
pub(crate) fn get_shell_completion(args: &Cli) -> io::Result<bool> {
  ...
  fn generate<G: Generator>(g: G) -> Vec<u8> {
    gen_completion(g)
  }
  ...
  let v = {
    ...
    match sh_name.as_str() {
      "bash" => generate(Bash),
      "zsh" => generate(Zsh),
      "pwsh" | "powershell" => generate(PowerShell),
      "fish" => generate(Fish),
      "elvish" => generate(Elvish),
      _ => {
        info!(
          "system shell: {:?}",
